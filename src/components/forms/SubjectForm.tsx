...
const SubjectForm = ({ type, data }: { type: "create" | "update"; data?: any }) => {
  ...
  const onSubmit = handleSubmit(async (formData) => {
    ...
    try {
      if (type === "create") {
        const { error } = await supabase.from("subjects").insert({ name: formData.name, code: formData.code || null, school_id: user.schoolId });
        if (error) throw error;
        setMsg("Subject created!");
      } else {
        const { error } = await supabase.from("subjects").update({ name: formData.name, code: formData.code || null }).eq("id", data?.id);
        if (error) throw error;
