...
const AssignmentForm = ({ type, data }: { type: "create" | "update"; data?: any }) => {
  ...
  return (
    <form className="flex flex-col gap-8" onSubmit={onSubmit}>
      <h1 className="text-xl font-semibold">{type === "create" ? t("form.createTitle", { entity: t("form.entities.assignment") }) : t("form.updateTitle", { entity: t("form.entities.assignment") })}</h1>
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <InputField label={t("form.fields.dueDate")} name="due_date" type="date" defaultValue={data?.due_date} register={register} error={errors?.due_date} />
      </div>
