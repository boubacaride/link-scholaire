-- ═══════════════════════════════════════════════════════════════════
-- 040 · Attendance & Performance — drill-down read RPCs
-- ───────────────────────────────────────────────────────────────────
-- Parameterized, server-side aggregation for the admin drill-down
-- (School → Grade level → Class → Student). One RPC per drill target so
-- the client never pulls the whole school down to filter locally.
--
-- All three are SECURITY DEFINER, scoped to the caller's school via
-- auth_school_id(), and gated on is_admin() (migration 003). They compute
-- live (not from snapshots):
--   • ACADEMIC AVERAGE = flat unweighted mean of mark %s (Σ(score/max·100)/n)
--   • ATTENDANCE RATE  = (present + late) / (present + late + absent + excused)
--   • grade level       = student's most-recent student_classes enrollment
-- Optional p_start / p_end (NULL = all-time) bucket grades by created_at and
-- attendance by date. Thresholds: pass 60, excellence 80, attendance target 90.
-- ═══════════════════════════════════════════════════════════════════


-- ─── LEVEL 1 — grade levels ───────────────────────────────────────────
...
-- ─── LEVEL 2 — classes within a grade level ───────────────────────────
