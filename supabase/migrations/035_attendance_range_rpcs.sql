-- ═══════════════════════════════════════════════════════════════════
-- 035 · Attendance: range RPCs + class_status fix
-- ───────────────────────────────────────────────────────────────────
-- The admin dashboard now offers a range dropdown (Today / Week /
-- Month / Year-to-date). The original single-day RPCs are replaced
-- by range-based versions; pass the same value for start + end to
-- get a one-day report.
--
-- Also fixes a typo in the previous attendance_class_status() body
-- (the `agg` CTE was joined twice, with an unused `a2` alias).
-- ═══════════════════════════════════════════════════════════════════


-- ─── Per-grade roll-up over a date range ─────────────────────────
-- absent / late / excused / present are INSTANCE counts — each
-- (student, day, absent) row counts once. distinct_students_absent
-- is unique students absent on AT LEAST one day in the range; the
-- UI uses it as the numerator for "% of students absent" so a
-- chronic absentee doesn't double-count.
DROP FUNCTION IF EXISTS public.attendance_school_day(DATE);
DROP FUNCTION IF EXISTS public.attendance_school_range(DATE, DATE);
...
BEGIN
  ...
  RETURN QUERY
  WITH enrollment AS (
    -- Distinct (student, grade); a student in multiple classes of the
    -- same grade counts once.
...
GRANT EXECUTE ON FUNCTION public.attendance_school_range(DATE, DATE) TO authenticated;


-- ─── Per-class live submission status — stays single-day ─────────
-- A "submitted" pill only makes sense for one day at a time; the
-- live feed is for the day the admin is looking at. Cleaned-up
-- version of the function from migration 034 (drops the unused
-- duplicate agg join).
...
DROP FUNCTION IF EXISTS public.staff_attendance_range(DATE, DATE);
...
GRANT EXECUTE ON FUNCTION public.staff_attendance_range(DATE, DATE) TO authenticated;


-- ─── Per-grade class submission summary (for cards) ──────────────
-- Returns, for ONE day, how many classes in each grade have already
-- submitted attendance. Drives the "Classes submitted" card so it
-- can break down by grade.
DROP FUNCTION IF EXISTS public.attendance_class_submission_by_grade(DATE);
...
GRANT EXECUTE ON FUNCTION public.attendance_class_submission_by_grade(DATE) TO authenticated;
